...
One other nice thing about individual settlement is that it doesn't global consensus or third party validation. You just need the two parties involved in the transaction, the User and the Relayer, to agree that the order was filled. This allows even more aggressive with settlement time. In Relay, actors will be incentivized (through reputation, bond penalties, etc) to quickly “self-settle” orders and only rely on 3rd party validation to settle disputes. This can result in even greater capital efficiency for Relayers.
