...
Relay shares the same core components as above: escrow, fulfillment, settlement, and payment as other escrow systems. However, it changes who, how and where they are executed, to improve efficiency.
...
The biggest unlock from this is that because the Relayer’s escrowed funds act as a bond, the User and Relayer can safely make direct transfers to each other, reducing the Deposit and Fill to highly efficient direct transfers (~21,000 gas each). This makes it up
...
- **Capital efficiency** - Relayer funds are tied up in between batches (4 hours for Across)
- **Flexibility** - All orders must use the same settlement mechanism
...
One other nice thing about individual settlement is that you don’t actually need any global consensus or third party validation. You really just need the two parties involved in the transaction, the User and the Relayer, to agree that the order was filled. This allows you to get even more aggressive with settlement time. In Relay, actors will be incentivized (through reputation, bond penalties, etc) to quickly “self-settle” orders and only rely on 3rd party validation to settle disputes. This can result in even greater capital efficiency for Relayers.
...
The way it works is that fees are paid out of the Relayer’s escrowed balance. For example, in a scenario where the user is bridging 1 ETH, the Relayer is charging 0.001, and other actors charging 0.002:
