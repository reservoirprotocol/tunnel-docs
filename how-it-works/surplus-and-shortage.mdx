...
## When would the operation fail?
We went over how if the quote can remain the same, then the swap or bridge will complete.
If the origin operation cannot receive at minimum the quoted amount, the operation will be cancelled & user refunded.
