...
When the user accepts the order, she submits a transfer to the solver. An order id associated with the quote is appended in the tx calldata. This order id will be used on the destination chain to link fills across chains.
