...
Swapping is the act of taking one token and exchanging it for another. There are many protocols that help with, notably, UniSwap, PancakeSwap, CowSwap, and many more. These protocols can either center around decentralized liquidity or centralized liquidity, with various different caveats for different use cases.
...
Relay builds on top of our pre-existing bridging infrastructure to allow cross-chain swapping. Our bridging infrastructure allows the fast transfer of native and some preset erc20 currencies cross-chains. 
...
- If its an erc20 input:
	- Approve permit2 on token, if first time ever swapping that token
	- Sign an approval
