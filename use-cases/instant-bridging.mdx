...
Relay's instant bridging is powered by cross-chain relaying. This model results in low-cost, low-latency (1-10 seconds) cross-chain transactions. Today, users of Relay work with a third-party provider for cross-chain relaying, such as Reservoir. Third parties escrow funds and take on reputational risk in the case of cross-chain disputes. Learn more about the [Technical Details of Relay](/what-is-relay).

## Integrating Instant Bridging

Relay is directly integrated into the Reservoir API & SDK. To get started make sure that you install and configure the [Reservoir SDK](https://docs.reservoir.tools/reference/reservoir-sdk-jstsnode) or [ReservoirKit](https://docs.reservoir.tools/reference/reservoirkit) (which exposes a client that you can tap into using the [useReservoirClient](https://docs.reservoir.tools/reference/reservoirkit-hooks#usereservoirclient) hook). In this example we'll be using the Reservoir SDK. Ensure that everything is configured properly before moving on to implementing the call action.
