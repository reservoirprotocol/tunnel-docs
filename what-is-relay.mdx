...
Today, Relay is powered by [Reservoir](https://reservoir.tools/), a trusted party that performs cross-chain relays and order validation. When a user wishes to bridge, they submit an order and a transfer to the Reservoir relayer, who validates the order and sends the funds to the user on the destination chain. Reservoir maintain a balance on all chains in order to make executions, and then periodically rebalances to ensure there is sufficient liquidity on all chains. In the rare case of a transaction failure, Reservoir refunds the user directly as soon as possible.
...
No! Today, Relay requires that you trust Reservoir, who is responsible for both order validation & relaying. We have mechanics in place to make sure orders are filled, and funds allocated in case of transaction failure. If you are interested in an enterprise agreement, please reach out.
