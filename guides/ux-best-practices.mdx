...
- **Supported Chains** - If the user needs to transact on multiple chains, they are better executing on the fly, to avoid needing to maintain a balance on every chain

Of course, you can always do both. We recommend making cross-chain execution the default, to minimize the friction for the user to have an "aha moment", and then later suggesting that they bridge if they are frequently transacting on a particular chain
...
- **Chain selector with fees** - Go one step further and show what the cost will be on each chain. While it's nice to give users' this information, in many cases it can also be overkill. 
