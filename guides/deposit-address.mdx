...
A deposit address is linked to a single bridge request. They are generated on-demand, and they cannot be reused. To enable using a deposit address for your bridge, you will need to set the `useDepositAddress` body parameter to `true` in the quote request. You will also need to specify the `refundTo` address, this is the address
...
- When non-supported currency or NFTs are sent to the deposit address, we
  do not support recovering of those assets.
