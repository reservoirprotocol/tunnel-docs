...
description: How to fetch a quote with the `getBridgeQuote` method
...
You can get the solver's capacity with the sdk's `getSolverCapacity` method. Optionally, you can call the [Get Config](references/api/get-config) api directly.
...
In the above example, we fetch the quote to bridge 0.1 ETH from Ethereum Mainnet to Zora. In the quote object we'll get data on the fees required to do this bridge. `quote.fees` gives use these values: 
