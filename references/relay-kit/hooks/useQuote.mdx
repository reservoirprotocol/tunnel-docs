...
The hooks returns an object with the base [Tanstack Query response](https://tanstack.com/query/v5/docs/framework/react/reference/useQuery), it also returns an `executeQuote` function which allows you to conveniently execute the quote. The function takes one parameter, an [onProgress callback](/references/sdk/actions/execute) that mirrors the underlying execute action. from the SDK.
...
const {
    ...
    walletClient.data,
    {
      ...
      originCurrency: "0x000000000bu0000000000000000000",
