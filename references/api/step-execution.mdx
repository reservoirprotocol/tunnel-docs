...
<Warning>
  We recommend using the [SDK](/references/sdk/getting-started) over our APIs.
  The SDK will iterate the steps and return useful callback information.
</Warning>
...
<Steps>
  <Step title="Fetching Steps" icon="server">
    Call the execute API ([call api](/references/api/execute-call)) or the
    ([bridge api](/references/api/execute-bridge))
  </Step>
...
Along with the steps you'll see that the following objects are returned: fees, breakdown and balances. Information about fees is detailed [here](/references/sdk/fetching-a-quote#fetching-a-quote). Breakdown pertains to time estimation for the execution, broken down by value. The balances object is in regards to the user and how much they require to solve for the execution.
